...
stadef patience_sort_index_t (len : int, i : int) =
  len == 0 || (0 <= i && i < len)
typedef patience_sort_index_t (tk : tkind, len : int, i : int) =
  [patience_sort_index_t (len, i)]
  g1uint (tk, i)
typedef patience_sort_index_t (tk : tkind, len : int) =
  [i : int]
  patience_sort_index_t (tk, len, i)

(* A type used in temporary workspaces. *)
stadef patience_sort_link_t (len : int, i : int) =
  0 <= i && i <= len
typedef patience_sort_link_t (tk : tkind, len : int, i : int) =
  [patience_sort_link_t (len, i)]
  g1uint (tk, i)
typedef patience_sort_link_t (tk : tkind, len : int) =
  [i : int]
...
in
  ...
  patience_sort_deal_refparams
            ...
            (arr    : &RD(array (a, n)),
             len    : g1uint (tk, len),
             piles  : &array (link_t (tk, len)?, len)
                      >> array (link_t (tk, len), len),
             links  : &array (link_t (tk, len)?, len)
                      >> array (link_t (tk, len), len))
  ...
  patience_sort_deal_valparams
            ...
            (pf_piles : !array_v (link_t (tk, len)?, p_piles, len)
                        >> array_v (link_t (tk, len), p_piles, len),
             pf_links : !array_v (link_t (tk, len)?, p_links, len)
                        >> array_v (link_t (tk, len), p_links, len) |
             arr      : &RD(array (a, n)),
             len      : g1uint (tk, len),
             p_piles  : ptr p_piles,
             p_links  : ptr p_links)
  ...
  patience_sort_merge_refparams
            ...
            (pf_exp2     : [exponent : nat] EXP2 (exponent, power) |
             ...
             workspace   : &array (link_t (tk, len)?, n_workspace)
                           >> _,
  ...
  patience_sort_merge_valparams
            ...
            (pf_exp2      : [exponent : nat] EXP2 (exponent, power),
             ...
             pf_links     : !array_v (link_t (tk, len), p_links, len)
                            >> _,
             pf_workspace : !array_v (link_t (tk, len)?, p_workspace,
                                      n_workspace) >> _ |
             arr          : &RD(array (a, n)),
             len          : g1uint (tk, len),
             num_piles    : g1uint (tk, num_piles),
